...
class DataSynchronizationManager: NSObject {
    ...
    func senđData(dataToBeSent: String) {
        ...
        if session.connectedPeers.count > 0 {
            do {
                try self.session.send(dataToBeSent.data(using: .utf8)!, toPeers: session.connectedPeers, with: .reliable)
            } catch let error {
...
extension DataSynchronizationManager : MCNearbyServiceAdvertiserDelegate {
...
extension DataSynchronizationManager : MCNearbyServiceBrowserDelegate {
...
extension DataSynchronizationManager : MCSessionDelegate {
